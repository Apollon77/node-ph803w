...
class PH803WDevice extends EventEmitter {
    ...
    sendAndRegisterForResponse(reqBuffer, responseType, responseTimeout) {
        if (!this.socket) {
            return;
        }
    ...
    resolveResponse(responseType, result) {
        ...
        debug(`Resolve for message type ${responseType}: ${result}`);
