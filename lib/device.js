...
class PH803WDevice extends EventEmitter {
    ...
    _handleData(data) {
        ...
        const dataLength = data[4] + 5;
        const messageType = data[7];

        if (data.length !== dataLength) {
            if (data.length > dataLength) {
                const additionalLength = data.length - dataLength;
                const additionalData = Buffer.alloc(additionalLength);
                const currentData = Buffer.alloc(dataLength);
                data.copy(currentData, 0, 0, dataLength);
                data.copy(additionalData, 0, dataLength);
                data = currentData;
                debug(`Split into two data packages because additional data detected. First ${data.toString('hex')} - Second ${additionalData.toString('hex')}`);
    ...
    _handleDataResponse(data) {
        ...
        const res = {
            ...
            ph,
            redox,
            phOutlet: ((data[9]>>0) % 2 !== 0),
            redoxOutlet: ((data[9]>>1) % 2 !== 0)
        };
    ...
    _handleDataExtendedResponse(data) {
        ...
        const res = {
            ...
            ph,
            redox,
            phOutlet: ((data[13]>>0) % 2 !== 0),
            redoxOutlet: ((data[13]>>1) % 2 !== 0)
        };
        debug(`Retrieved data: ${JSON.stringify(res)}`);
        this.emit('data', res);
