...
class PH803WDiscovery extends EventEmitter {
    ...
    discover() {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.socket.bind(undefined, this.options.listenAdress || '0.0.0.0', () => {
                if (!this.socket) {
                    return;
                }
    ...
    parseResponse(data, remote) {
        ...
        const apiServer = this.readDataField(data, dataIdx, false, true);
        dataIdx += apiServer.length + 1;
        const version = this.readDataField(data, dataIdx, false, true);
